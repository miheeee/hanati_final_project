      ...
      <div class="container">
        ...
        <div class="row justify-content-center mb-1 element-animate">
          <div class="col-md-8 text-center mb-5" >
            <h2 class="text-uppercase heading border-bottom mb-4" style="font-weight:600;">${loginVO.name}님의 모임 계좌 현황</h2>       
	        <c:choose>
		      	<c:when test="${not empty gatheringList}">
		 			<p class="mb-0 lead" style="font-weight:500;">총 ${fn:length(gatheringList)}개의 모임 가입 중</p>
		      	</c:when>
		      	<c:otherwise>
        ...
        <!-- 시작 slider -->
          ...
          <c:choose>
	             ...
	             <div class="media d-block media-feature text-center">
		             ...
		             <div class="media-body">
		          		...
		          		<c:choose>
		                	...
		                	<c:otherwise>
		                		<h3 class="mt-0 text-black">${gathering.name}</h3> 
		                		<p>${fn:substring(gathering.safeAccountNo, 0, 4)}-${fn:substring(gathering.safeAccountNo, 4, 6)}-${fn:substring(gathering.safeAccountNo, 6, 11)}</p>	<%-- <fmt:formatDate value="${gathering.safeAccountNo}" pattern="####-##-#####" />  --%>
		                		<h3 class="mt-0 text-black"><fmt:formatNumber value="${gathering.balance}" pattern="#,###.##"/>원</h3>
		                	</c:otherwise>
		               	</c:choose>
		               	<form action="${ pageContext.request.contextPath }/gathering/detail" method="post" style="display:inline">
        	...
        	<c:otherwise>
	 			...
	 			<!-- 가입한 모임통장이 하나도 없으면 +표시로 모임통장 신청폼으로 이어지는 링크 연결하기 -->
	             <div class="media d-block media-feature text-center">
